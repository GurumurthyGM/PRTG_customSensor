...
for proc in NMSProcs:
    ...
    elif "/opt/nms/release/apache-tomcat-8.0.30/MTOSI/conf/logging.properties" in proc:
        process["mtosi"] = proc.split()[0]
...
for proc in processlist:
    ...
    elif process['mtosi'] in proc:
        procusage['mtosi'] = dict(zip(('cpu','mem'),cpumem.findall(proc)[0]))
...
for procdb in processlistdb:
    if mysql in procdb:
        procusage['mysql'] = dict(zip(('cpu','mem'),cpumem.findall(procdb)[0]))
